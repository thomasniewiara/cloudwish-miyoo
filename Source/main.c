...
static void backup_legacy(void){
 ...
 if(strcmp(tag,"CW1")&&strcmp(tag,"CW2")){fclose(f);return;}rewind(f);
 if(access("saves/pre-0.3-backup.txt",F_OK)==0){fclose(f);return;}
 FILE*out=fopen("saves/pre-0.3-backup.txt","w");if(out){int c;while((c=fgetc(f))!=EOF)fputc(c,out);fclose(out);}fclose(f);
}
int main(int argc,char**argv){
 ...
 if(argc>2&&!strcmp(argv[1],"--screenshot")){
  ...
  if(argc>4){if(!strcmp(argv[4],"shop"))mode=SHOP;else if(!strcmp(argv[4],"map"))mode=MAP;else if(!strcmp(argv[4],"decor"))mode=DECOR;else begin_spell(atoi(argv[4])%4);}
